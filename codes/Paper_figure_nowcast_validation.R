...
# Load files
validation_files <- list.files('data/validation_data/', full.names = TRUE)
validation_files <- validation_files[grep('v2_|v3_', validation_files)]
csvList <- lapply(validation_files, read.csv)
names(csvList) <- gsub('.*\\/|\\.csv', '', validation_files)
list2env(csvList, envir = .GlobalEnv)
...
# save plot
ggsave(filename = 'figures/v2_vs_v3.pdf', height = 7, width = 10, plot = p2)
