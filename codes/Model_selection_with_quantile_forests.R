# Code to run model selection

# load covariates to test
...
results_dir <- 'data/model_selection/'

# list smote datafiles
filenames <- list.files("data/smote_data/", full.names = TRUE)
...
for(i in length(filenames):1){
  ...
  # set disease name
  dz_name <- substr(filenames[i], 17, 22)
